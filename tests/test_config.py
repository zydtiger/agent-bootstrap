...
def _write_config(root: Path, manifest: str) -> Path:
    ...
    (root / "cursor.md").write_text("cursor\n", encoding="utf-8")
...
""",
    ...
    with pytest.raises(ManifestError, match="unknown agent 'gemini'"):
        load_manifest(path)
...
def test_loads_schema_v3_cursor_agent_and_intersection_fragments(tmp_path: Path) -> None:
    path = _write_config(
        tmp_path,
        """
schema_version = 3
...
""",
    ...
    assert [
        manifest.display_path(item) for item in manifest.fragments_for("workstation", Agent.CURSOR)
    ] == ["shared.md", "cursor.md", "host.md", "host-agent.md"]
    assert [
        manifest.display_path(item) for item in manifest.fragments_for("workstation", Agent.CODEX)
    ] == ["shared.md", "agent.md", "host.md"]
...
[host_agents.workstation.gemini]
fragments = ["host-agent.md"]
""",
            "unknown agent 'gemini' in host_agents.workstation",
        ),
        (
            """
schema_version = 3
